...
static void _BGM_define_coord_range(
        int          min,
        int          max,
        int          val,
...
static void _BGM_define_angle_range(
        unsigned int  val,
        unsigned int  tolerance,
        unsigned int *begin,
        unsigned int *end)
...
static void _BGM_reset_score(struct _BGM_global_score *score)
{
    memset(
     score->minutiae_scores, 0,
     score->num_templates * MAX_MINUTIAE_PER_TEMPLATE_DFL * sizeof(unsigned int)
          );
}

static BGM_status _BGM_create_score(struct BGM_context *ctx, struct _BGM_global_score *score)
{
    BGM_status status;
    unsigned int alloc_size;

    // TODO: Stop using MAX_MINUTIAE_PER_TEMPLATE_DFL directly.
    alloc_size = ctx->db.next_template_id * MAX_MINUTIAE_PER_TEMPLATE_DFL * sizeof(unsigned int);
...
static void _BGM_destroy_score(struct _BGM_global_score *score)
{
    free(score->minutiae_scores);
}

static void _BGM_calc_compatibility_window(struct BGM_context   *ctx,
...
void _BGM_update_score_from_group(
...
void _BGM_update_score_with_matching_neighbors(
        struct BGM_context       *ctx,
        struct _BGM_neighbor     *nei,
        struct _BGM_global_score *score)
{
    ...
    _BGM_calc_compatibility_window(ctx, nei, &start_x, &end_x, &start_y, &end_y, &start_t, &end_t);

    for (int x = start_x; x <= end_x; x+=ctx->db_cfg.pixels_per_group) {
        for (int y = start_y; y <= end_y; y+=ctx->db_cfg.pixels_per_group) {
            for (unsigned int t = start_t; t <= end_t; t+=ctx->db_cfg.degrees_per_group) {
                unsigned int group_index;
                status = _BGM_calc_group_index(ctx, x, y, t%360, &group_index);
...
void _BGM_get_matching_id(struct BGM_context *ctx,
                          struct _BGM_global_score *score,
                          unsigned int *tpl_id)
{
    ...
    if (template_scores != NULL) {
        ...
        for (unsigned int i = 0; i < score->num_templates * MAX_MINUTIAE_PER_TEMPLATE_DFL; i++) {
            if (score->minutiae_scores[i] != 0) {
                if (score->minutiae_scores[i] >= ctx->match_cfg.minutia_threshold) {
...
BGM_status _BGM_identify(struct BGM_context *ctx, struct BGM_template *tpl, bool *found, unsigned int *tpl_id)
{
    BGM_status status;
    struct _BGM_global_score score;
    ...
    *found = false;
    ...
    if (status == BGM_SUCCESS) {
        for (unsigned int min_index = 0; min_index < tpl->num_minutiae; min_index++) {
            for (unsigned int nei_index = 0; nei_index < tpl->minutiae[min_index].num_neighbors; nei_index++) {
...
BGM_status BGM_identify(struct BGM_context *ctx, struct BGM_template *tpl, bool *found,
                        unsigned int *tpl_id)
{
    BGM_status status;
